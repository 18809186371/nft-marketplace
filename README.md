...
项目特点
完整的 NFT 铸造流程（图片上传至 IPFS，元数据生成）

市场合约支持上架、购买、取消上架

个人页面展示用户拥有的 NFT 和已上架的 NFT

使用 Alchemy 获取链上数据，Etherscan 验证合约

使用 Filebase 作为 IPFS 存储服务
...
环境变量配置
根目录 .env 文件
在项目根目录创建 .env 文件，用于 Hardhat 部署和验证：
...
获取方式：
SEPOLIA_RPC_URL: 登录 Alchemy → 创建新 App（选择 Ethereum Sepolia）→ 复制 HTTPS 端点。

SEPOLIA_PRIVATE_KEY: 从 MetaMask 导出账户私钥（设置 → 安全与隐私 → 导出私钥），注意：测试网私钥相对安全，但请勿用于主网。

ETHERSCAN_API_KEY: 登录 Etherscan → 右上角用户名 → API Keys → 创建新密钥。
...
注册 Filebase。
创建存储桶（Bucket），记下桶名称。
在“Access Keys”页面生成 Access Key 和 Secret Key。
